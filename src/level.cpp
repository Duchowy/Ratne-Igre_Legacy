...
state_change_limit process_riven(riven *mod, state_change_limit *base)
{
    ...
    for (int i = 0; i < (mod->dualSided ? 4 : 2); i++)
    {
        ...
        {
        case 1:
            overwrite.speed_limit[1] *= mod->value[i];
            break; // uper speed limit
        case 2:
            overwrite.speed_limit[0] *= 2. - mod->value[i];
            break; // lower speed limit
        case 3:
            overwrite.speed_limit[1] *= mod->value[i];
            overwrite.speed_limit[0] *= 2. - mod->value[i];
            break;
        case 4:
            overwrite.speed_rate[1] *= mod->value[i];
            break;
        case 5:
            overwrite.speed_rate[0] *= mod->value[i];
            break;
        case 6:
            overwrite.speed_rate[1] *= mod->value[i];
            overwrite.speed_rate[0] *= mod->value[i];
            break;
        case 7:
            overwrite.alter.turn_speed += mod->value[i];
            break;
        case 8:
            overwrite.turn_rate += mod->value[i];
            break;
        case 9:
            overwrite.default_speed += mod->value[i];
            break;
        case 10:
            overwrite.mobility_coef -= mod->value[i];
            break;
        }
        // printf("%f\n",mod->value[i]);
    }
...
void refresh_riven(struct LevelInst *level, struct asset_data *asset)
{
    for (int i = 0; i < ENUM_JET_TYPE_FIN; i++)
    {
        ...
        {
            if (!level->player.custom_stat[i])
                level->player.custom_stat[i] = new state_change_limit;
...
riven *spawn_riven()
{
    ...
    do
    {
        ...
        if (!unique)
            mod.type[pos] = (mod.type[pos] + 1 > 10 ? 1 : mod.type[pos] + 1);
        else
            pos += 1;
...
#pragma omp parallel for
    for (std::vector<JetInst>::iterator object = input_vec.begin(); object != input_vec.end(); object++)
    {
        ...
        {
            if (object->weapon[i].cooldown)
                object->weapon[i].cooldown--;
            if (object->weapon[i].replenish_cooldown)
                object->weapon[i].replenish_cooldown--;
        }
        if (object->ability)
        {
            for (int i = 0; i < ENUM_BOSS_ABILITY_FIN; i++)
            {
                if (object->ability[i].cooldown)
                    object->ability[i].cooldown--;
                if (object->ability[i].duration)
                    object->ability[i].duration--;
            }
        }
    }
}
...
#pragma omp parallel for
    for (std::vector<ProjInst>::iterator object = level->proj_q.begin(); object != level->proj_q.end(); object++)
        object->decay = (object->decay - 1 > 0 ? object->decay - 1 : 0);
#pragma omp parallel for
...
void boss_ability_use(asset_data *asset, LevelInst *level)
{
    for (std::vector<JetInst>::iterator object = level->jet_q.begin() + 1; object != level->jet_q.end(); object++)
    {
        ...
        {
            if (asset->boss_data[object->type - ENUM_JET_TYPE_FIN].ability[RAND_POS])
                randomize_position(object, asset, level);
            if (asset->boss_data[object->type - ENUM_JET_TYPE_FIN].ability[DASH])
                dash(object, asset->lvl_data[level->level_name].map_width, asset->lvl_data[level->level_name].map_height);
            if (asset->boss_data[object->type - ENUM_JET_TYPE_FIN].ability[CMEASURE])
            ...
            if (object.status[BURNING])
            {
                ...
                for (int i = 0; i < num; i++)
                {
                    ...
                    ParticleInst temp1 = {
                        .type = PIXEL,
                        .bitmap = nullptr,
                        .color = new sf::Color(127, 0, 127, 255),
                        .curr = {
                            .x = object.curr.x,
                            .y = object.curr.y,
                            .turn_angle = angle_addition(object.curr.turn_angle, (float)rand() / RAND_MAX * (GenerateLeft ? -PI / 30 : PI / 30)),
...
void process_radar(LevelInst *level)
{
    ...
    if (follow_target == level->jet_q.end())
        player->botTarget = -1;
    if (fabs(rad_distance(&player->curr, &follow_target->curr)) > level->radar.range_rad || distance(&player->curr, &follow_target->curr) > level->radar.range_dist)
        player->botTarget = -1;
...
void render(struct LevelInst *level, struct asset_data *asset, sf::RenderWindow &display)
{
    ...
    camera.setCenter(level->jet_q.front().curr.x,level->jet_q.front().curr.y);
...
int level(sf::RenderWindow & display, asset_data *assets, LevelInst *lvl)
{
    ...
    while (!kill)
    {
        ...
        while(display.pollEvent(event))
        {
            ...
            {
                sf::View resized = display.getView();
                resized.setSize(event.size.width,event.size.height);
                display.setView(resized);

                if (assets->config.autoUIscale)
                    assets->config.UIscale = calculateUIscale(event.size.width, event.size.height);
            ...
            case sf::Event::MouseWheelScrolled:
            {
                ...
                {
                    zoom(display,assets, event.mouseWheelScroll.delta);
                }
                break;
            }
            case sf::Event::KeyReleased:
            {
                switch(event.key.code)
                {
                    case sf::Keyboard::F:
                    {
                        ...
                        {
                            if (lvl->prompt_q.back().type == 1)
                                destroy_prompt_screen(assets, lvl, lvl->prompt_q.end() - 1, 1);
                        }
                    }
                    break;
                    case sf::Keyboard::Z:
                    {
                        ...
                        {
                            if (lvl->prompt_q.back().type == 1)
                                destroy_prompt_screen(assets, lvl, lvl->prompt_q.end() - 1, 0);
                        }
                    }
                    break;
                    case sf::Keyboard::R:
                    {
                        ...
                        if (lvl->radar.range_rad > 0)
                            lvl->radar.range_rad = new_range_rad;
                        else
                            lvl->radar.range_rad = -new_range_rad;

                        if (fabs(lvl->radar.turn_angle) > new_range_rad)
                        {
                            if (lvl->radar.turn_angle > 0)
                                lvl->radar.turn_angle = new_range_rad;
                            else
                                lvl->radar.turn_angle = -new_range_rad;
                        }
            ...
            {
                if(event.key.code == sf::Keyboard::Space) lvl->jet_q.front().weapon[1].engaged = 1;
                ...
                {
                    if (lvl->radar.mode == 2)
                        radarCycleTarget(lvl->jet_q, lvl->jet_q.begin(), &lvl->radar);
                }
            }
            default: break;
            }
...

        {
            ...
            { // player actions
                ...
                if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
                    lvl->jet_q.front().weapon[0].engaged = 1; // left mouse button
                if (sf::Mouse::isButtonPressed(sf::Mouse::Right))
                    lvl->jet_q.front().weapon[2].engaged = 1; // right mouse button
                lvl->jet_q.front().alter.target_angle = atan2((mouse.y - lvl->jet_q.front().curr.y), (mouse.x - lvl->jet_q.front().curr.x));

                bool accelerate = sf::Keyboard::isKeyPressed(sf::Keyboard::W);
                bool decelerate = sf::Keyboard::isKeyPressed(sf::Keyboard::S);
            ...
            for (int i = 1; i < 3; i++)
                lvl->jet_q.front().weapon[i].engaged = 0;
